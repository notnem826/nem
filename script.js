...
const quoteDatabase = [
    ...
    },
    {
        ...
        ja: "「打ち倒されたかどうかではない。立ち上がったか do が問題なのだ。」 -Vince Lombardi"
    },
    {
...
// Danh sách 6 GIF Anime đổi ngẫu nhiên
const avatarGifs = [
...
// Danh sách nhạc Chill Chill cho phép tự động phát ngẫu nhiên
const chillTracks = [
...
function openProfile() {
    // Hiệu ứng Hào quang Aura trắng sáng màn hình
    ...
    // Âm lượng mặc định 50% & phát nhạc
    if (audio) {
        audio.volume = 0.5;
...
function resetAvatarRotation() {
    const avatarImg = document.getElementById('musicRandomAvatar');
    if (avatarImg) {
        // Reset animation xoay vòng tròn từ ban đầu
...
function toggleMusic() {
    ...
    if (isPlaying) {
        ...
        audio.play();
        if (playIcon) playIcon.className = 'fas fa-pause';
        resetAvatarRotation(); // Bắt đầu lại vòng xoay từ ban đầu khi bấm Play
    }
    isPlaying = !isPlaying;
}
...
function changeTrack(src) {
    ...
    // Đổi GIF ngẫu nhiên khi chuyển bài
    changeRandomAvatar();
    resetAvatarRotation();
...
// TỰ ĐỘNG CHUYỂN BÀI NGẪU NHIÊN KHI HẾT BÀI (CHỈ TRONG CÁC BÀI CHILL, KHÔNG CHẠY CÁC BÀI HARDSTYLE)
if (audio) {
